...
class GoogleMapPlotter(object):
  ...
  #      self.coloricon = "C:\Python27\Lib\site-packages\gmplot\markers\%s.png"
    ...
    def _process_kwargs(self, kwargs):
        ...
        settings["edge_width"] = kwargs.get("edge_width", None) or \
                                 ...
                                 1.0
        settings["face_alpha"] = kwargs.get("face_alpha", None) or \
                                 kwargs.get("face_alpha", None) or \
                                 kwargs.get("fa", None) or \
                                 0.3
        settings["face_color"] = kwargs.get("face_color", None) or \
        ...
        settings["name"] = kwargs.get("name", None) or "polygon"
    ...
    def polygon(self, lats, lngs, color=None, c=None, name="polygon", title=None, **kwargs):
        color = color or c
        kwargs.setdefault("color", color)
        kwargs["title"]=title
        kwargs["name"]=name
    ...
    # J add https://divideandconquer.se/2011/09/15/marker-and-polygon-tooltips-in-google-maps-v3/
        f.write('function attachPolygonInfoWindow(polygon, html)\n')
        f.write('{\n')
        f.write('polygon.infoWindow = new google.maps.InfoWindow({\n')
	f.write('content: html,\n')
	f.write('});\n')
	f.write('google.maps.event.addListener(polygon, "mouseover",\n')
	f.write('function(e) {\n')
	f.write('var latLng = e.latLng;\n')
	f.write('polygon.infoWindow.setPosition(latLng);\n')
	f.write('polygon.infoWindow.open(map);\n')
	f.write('});\n')
	f.write('google.maps.event.addListener(polygon, "mouseout",\n')
	f.write('function() {\n')
	f.write('polygon.infoWindow.close();\n')
	f.write('});\n')
	f.write('}\n')
    ...
    def write_polygon(self, f, path, settings):
        ...
        f.write('});\n')
    # J add https://divideandconquer.se/2011/09/15/marker-and-polygon-tooltips-in-google-maps-v3/
        f.write('attachPolygonInfoWindow(%s, "%s");\n' % (name, title))
#
        f.write('\n')
